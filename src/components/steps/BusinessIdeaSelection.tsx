...
export function BusinessIdeaSelection() {
  ...
  useEffect(() => {
    const fetchBusinessIdeas = async () => {
      if (state.businessIdeas.length === 0 && state.selectedPersona) {
        try {
          const response = await sendMessage('ビジネスアイデアを生成してください');
    ...
    fetchBusinessIdeas();
  }, [state.selectedPersona, state.businessIdeas.length, sendMessage, updateState]);
  ...
  if (loading && state.businessIdeas.length === 0) {
    return (
      <motion.div
  ...
  return (
    ...
    >
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-center mb-4">ビジネスアイデアを選択してください</h2>
        <p className="text-center text-muted-foreground">
          選択したペルソナ「{state.selectedPersona?.description}」に基づいて生成されたビジネスアイデアから選んでください。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {state.businessIdeas.map((idea, index) => (
          <motion.div
            key={idea.id}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            >
              <CardHeader>
                <div className="flex items-center space-x-2">
                  ...
                  <CardTitle className="text-lg">アイデア {idea.id}</CardTitle>
                </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {idea.osborneMethod && (
                    <div>
                      <h4 className="font-semibold text-sm">オズボーン手法</h4>
                      <p className="text-sm text-muted-foreground">{idea.osborneMethod}</p>
                    </div>
                  )}
                </div>
              </CardContent>
