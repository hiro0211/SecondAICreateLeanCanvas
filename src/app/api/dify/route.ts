...
async function callDifyWorkflow<T>(
  task: DifyTaskType,
  ...
  try {
    ...
    if (!DIFY_API_URL || !DIFY_API_KEY) {
      throw new Error("Dify API credentials are not configured properly");
    }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error("❌ Dify API error details:", errorDetails);
      } catch {
        // テキスト取得に失敗した場合はステータスのみ
      }
      throw new Error(
    ...
    // Difyからのレスポンスを安全にパース
    const parsedData = parseTaskResponse(
      task,
      data.data?.outputs?.result || data.data?.outputs
    );
